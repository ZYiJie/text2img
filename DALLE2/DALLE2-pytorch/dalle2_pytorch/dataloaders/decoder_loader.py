...
def embedding_inserter(samples, embeddings_url, index_width, sample_key='npy', handler=wds.handlers.reraise_exception):
    ...
    def load_corresponding_embeds(tar_url, embedding_url):
        # example: /data/imgs/00004.tar => 00004.npy
        _name = tar_url.split("/")[-1].split(".")[0] + ".npy"
        _embedding_path = os.path.join(embedding_url, _name)
        # print(_embedding_path)
        with embeddings_fs.open(_embedding_path) as f:
    ...
    for idx, sample in enumerate(samples):
        ...
        if tar_url != previous_tar_url:
            ...
            current_embeddings = load_corresponding_embeds(tar_url, embeddings_url)
        # print(idx, tar_url)
        embedding = torch.from_numpy(current_embeddings[key])
...
def unassociated_shard_skipper(tarfiles, embeddings_url, handler=wds.handlers.reraise_exception):
    ...
    get_embedding_shard = lambda embedding_file: int(embedding_file.split("/")[-1].split(".")[0])
...
def join_embeddings(samples, handler=wds.handlers.reraise_exception):
    ...
    for sample in samples:
        ...
        if 'text_encoding' in sample:
            sample['emb']['text_encoding'] = sample['text_encoding']
        yield sample
...
class ImageEmbeddingDataset(wds.DataPipeline, wds.compat.FluidInterface):
    ...
    def __init__(
        ...
        # if text_encoding_folder_url is not None:
        #     self.append(skip_unassociated_shards(embeddings_url=text_encoding_folder_url, handler=handler))
        ...
        if text_encoding_folder_url is not None:
            # Then we are loading image embeddings for a remote source
            assert index_width is not None, "Reading embeddings separately requires index width length to be given"
            self.append(insert_embedding(embeddings_url=text_encoding_folder_url, index_width=index_width, sample_key='text_encoding', handler=handler))
