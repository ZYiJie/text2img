...
class HuggingfaceAdapter(BaseClipAdapter):
    def __init__(
        ...
        self.text_encoder = BertModel.from_pretrained(text_encoder_name).eval()
        self.processor = CLIPProcessor.from_pretrained(img_encoder_name)
        self.img_encoder = CLIPModel.from_pretrained(img_encoder_name).eval()
    ...
    @property
    def image_size(self):
        return self.img_encoder.config.image_size
    ...
    def embed_text(self, text):
        ...
        text_encodings = outputs.last_hidden_state
        text_mask = text["attention_mask"].bool()
        # Todo: 是否要把这个masked_fill去掉
        text_encodings = text_encodings.masked_fill(~text_mask[..., None], 0.)
        return EmbeddedText(l2norm(text_embed.float()), text_encodings.float())

    @torch.no_grad()
    def embed_image(self, image):
        # print(111111, image.device, self.img_encoder.device, self.text_encoder.device)
        features = self.img_encoder.get_image_features(image)
        return EmbeddedImage(l2norm(features.float()), None)
