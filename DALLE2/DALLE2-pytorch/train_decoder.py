...
def generate_samples(trainer, example_data, clip=None, start_unet=1, end_unet=None, condition_on_text_encodings=False, cond_scale=1.0, device=None, text_prepend="", match_image_size=True):
    ...
    if condition_on_text_encodings:
        if text_embeddings[0] is None:
            ...
            tokenized_texts = clip.tokenizer(
                ...
                for unet in range(1, trainer.num_unets+1):
                    ...
                    if condition_on_text_encodings:
                        if has_text_embedding:
                            ...
                            tokenized_texts = clip.tokenizer(
                                txt, max_length=clip.max_text_len, 
                                padding='max_length', truncation =True, return_tensors="pt"
                            ).to(inference_device)
                    ...
                    #     print(f"loss={loss}\timg={img.shape} {img.sum()}")
                    ...
                    #     for k, v in forward_params.items():
                    #         print(k, v.shape, v.sum())
                    # assert torch.isnan(torch.tensor(loss)).sum() == 0
...
def initialize_training(config: TrainDecoderConfig, config_path):
    ...
    # print(2222, decoder.device, clip.text_encoder.device)
