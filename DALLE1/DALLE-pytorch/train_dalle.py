...
train_group.add_argument('--keep_n_checkpoints', default = 5, type = int, help = '(Careful) Deletes old deepspeed checkpoints if there are more than n')
...
model_group.add_argument('--text_seq_len', default = 32, type = int, help = 'Text sequence length')
...
if RESUME:
    ...
    print(111, vae_params, args.taming)
...
sample_sent = '我最爱吃红烧牛肉面了'
mysample = tokenizer.tokenize(sample_sent,TEXT_SEQ_LEN,truncate_text=args.truncate_captions).cuda()
...
for epoch in range(resume_epoch, EPOCHS):
    ...
    for i, (text, images) in enumerate((dl if ENABLE_WEBDATASET else distr_dl)):
        ...
        if i % int(len(dl if ENABLE_WEBDATASET else distr_dl)/2) == 0:  # 1 epoch save model 2 times
            save_model(DALLE_OUTPUT_FILE_NAME, epoch=epoch)

        if i % int(len(dl if ENABLE_WEBDATASET else distr_dl)/3) == 0 and is_root: # 1 epoch save image 3 times
            sample_text = text[:1]
            ...
            if not avoid_model_calls:
                ...
                image2 = dalle.generate_images(mysample, filter_thres=0.9)  # topk sampling at 0.9

            if not avoid_model_calls:
                log['image'] = wandb.Image(image, caption=decoded_text)
                log['mySample'] = wandb.Image(image2, caption=sample_sent)
                log['original_image'] = wandb.Image(tensor2image(images[0]), caption=decoded_text)
