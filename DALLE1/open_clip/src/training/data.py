...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, img_key, caption_key, tokenizer, sep="\t"):
    ...
    def __getitem__(self, idx):
        ...
        texts = self.tokenizer(str(self.captions[idx]), padding='max_length', 
                                    max_length = 34, truncation=True,
                                    return_tensors='np')['input_ids'][0]
        # print(222, texts)
        # print(333, len(str(self.captions[idx])), len(texts))
        # exit()
        return images, texts
